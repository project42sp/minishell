...
//static t_tree	*tree_redir(t_token **token, t_token *tk_right)
//{
//	t_tree	*left;
//	t_tree	*right;
//	t_tree	*tree_node;
...
//	tree_node = NULL;
//	if (!*token && tk_right)
//		return tree_node_create(NULL, &tk_right, NULL);
//	if (!*token)
//		return (NULL);
//	while ((*token)->signal == CMD)
//	{
//		right_node = collect_args(token);
//		if (!right_node)
//			return (NULL);
//	}
//	if (right_node && !tk_right)
//	{
//		tk_right = token_node((void *)right_node, CMD);
//		if (!tk_right)
//			return (NULL);
//		if (tk_right && !*token)
//			return tree_node_create(NULL, &tk_right, NULL);
//	}
//	if (*token && (*token)->signal >= INPUT && (*token)->signal <= HEREDOC)
//	{
//		redir = *token;
//		*token = (*token)->next;
//		left = tree_node_create(NULL, token, NULL);
//		right = tree_redir(token, tk_right);
//		tree_node = tree_node_create(left, &redir, right);
//	}
//	return (tree_node);
//}
